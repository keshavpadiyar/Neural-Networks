...
As displayed in the below figure there are data points belonging to 2 classes (in green and red) and it can be separable using a line or hyperplane. Hence a simple linear classifier is sufficient to discriminate the datapoints into its respective classes.

![Distribution of Data set 1](Data_1.png)
**DataSet 2 :**
...
![Distribution of Data set 2](Data_2.png)
**DataSet 3 :**
...
$\sum_{feature-1}^{feature-n}\sqrt{(Xnew_{i}-Xknown_{i})^2}$
...
```{r fig4,echo=FALSE}
knitr::include_graphics("knnData3.png")
```
...
```{r NNSD1,echo=FALSE}
knitr::include_graphics("NNSD1.png")
```


```{r NNSE1,echo=FALSE}
knitr::include_graphics("NNSE1.png")
```
**Dataset 2:**
...
```{r NNSD2,echo=FALSE}
knitr::include_graphics("NNSD2.png")
```


```{r NNSE2,echo=FALSE}
knitr::include_graphics("NNSE2.png")
```
...
```{r NNSD3,echo=FALSE}
knitr::include_graphics("NNSD3.png")
```


```{r NNSE3,echo=FALSE}
knitr::include_graphics("NNSE3.png")
```
**Dataset 4:**
...
```{r NNSD4,echo=FALSE}
knitr::include_graphics("NNSD4.png")
```

```{r NNSE4,echo=FALSE}
knitr::include_graphics("NNSE4.png")
```
...
```{r NNMD1,echo=FALSE}
knitr::include_graphics("NNMD1.png")
```


```{r NNME1,echo=FALSE}
knitr::include_graphics("NNME1.png")
```
...
```{r NNMD2,echo=FALSE}
knitr::include_graphics("NNMD2.png")
```


```{r NNME2,echo=FALSE}
knitr::include_graphics("NNME2.png")
```
...
```{r NNMD3,echo=FALSE}
knitr::include_graphics("NNMD3.png")
```


```{r NNME3,echo=FALSE}
knitr::include_graphics("NNME3.png")
```
From the above error curves it could be depicted that the model is slightly overfitted. However, the model has given estimated accuracy over test data.
...
```{r NNMD4,echo=FALSE}
knitr::include_graphics("NNMD4.png")
```

```{r NNME4,echo=FALSE}
knitr::include_graphics("NNME4.png")
```

Above error curve depicts that, for the set of tuned parameters for dataset 4, model overfits as number of iterations increases. But however its noticeable that, error curve of test data is getting flat and not increasing and meeting the expected accuracy level.
...
```{r fig6,echo=FALSE}
knitr::include_graphics("NN_8_Test.png")
```

```{r fig7,echo=FALSE}
knitr::include_graphics("NN_8_Error.png")
```
...
Data points in the OCR data are having pixel values ranging from 0 to 16, if normalize the data to range between -1 to 1. As its ideal/recommended to have data points in same scale while calculating Euclidean distance.

Also, for the neural network with non-linear activation function - *tanh(x)* the network was not converging if the data points after multiplying with the weights get too large. We observed this in the case of OCR data. Hence we decided to give very small initial weights, ranging within -1 to 1 e^-3. Alternative could be normalizing the OCR data.
