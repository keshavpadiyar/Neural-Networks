...
%DTrain = combineBins(DBins, [1,2,3,4]);
...
DTrain = DBins{1};
LTrain = LBins{1};
XTest  = XBins{2};%combineBins(XBins, 2:100);
DTest  = DBins{2};%combineBins(DBins, 2:100);
LTest  = LBins{2};%combineBins(LBins, 2:100);
...
V0 = randn(numHidden+1,size(DTrain,2))/100; % Initialize your weight matrix V (added +1 since we need to consider bias at 2nd layer)
